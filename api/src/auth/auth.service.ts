...
export class AuthService {
  ...
  async signup(
    ...
    try {
      ...
      if (err instanceof Neo4jError) {
        if (
          (err as Neo4jError).code ===
          'Neo.ClientError.Schema.ConstraintValidationFailed'
        ) {
          let user: Neode.Node<UserInterface> = await this.neode.first(
            'User',
            'username',
            username,
          );
  ...
  async login(loginCredentialsDTO: LoginCredentialsDTO) {
    ...
    const userInstance: Neode.Node<UserInterface> = await this.neode.first(
      'User',
      'email',
      email,
    );
